...
class SimulationEngine:
    ...
    def register_path(self, path: Path) -> None:
        ...
        for (prev_turn, prev_zone), (turn, zone) in zip(path, path[1:]):
            ...
            if turn - prev_turn == 1:
                self._reserve_connection(prev_zone, zone, turn)
            else:
                self._reserve_connection(prev_zone, zone, turn)
                self._reserve_connection(prev_zone, zone, turn - 1)
    ...
    def _reserve_zone(self, zone: str, turn: int) -> None:
        ...
        turn_zones = self.pathfinder.zone_reservations.get(turn, {})
        turn_zones[zone] = turn_zones.get(zone, 0) + 1
        self.pathfinder.zone_reservations[turn] = turn_zones

    def _reserve_connection(self, zone_a: str, zone_b: str, turn: int) -> None:
        key = self.pathfinder.connection_key(zone_a, zone_b)

        turn_conns = self.pathfinder.connection_reservations.get(turn, {})
        turn_conns[key] = turn_conns.get(key, 0) + 1
        self.pathfinder.connection_reservations[turn] = turn_conns

    def get_location_at_time(
        self, path: Path, target_time: int
    ) -> Optional[str]:
        """Where is the drone at `target_time`?

        Returns a zone name, or if `target_time` falls in the middle of
        a restricted-zone move, "zoneA-zoneB" for the connection it is
        currently crossing.
        """
    ...
    def print_simulation(self) -> None:
        """Prints one line per turn, listing every drone that moved."""
        if not self.all_paths:
            return

        max_turn = max(
            (path[-1][0] for path in self.all_paths.values() if path),
            default=0,
        )
        ...
        for current_turn in range(1, max_turn + 1):
            ...
            for zone_name, zone in self.graph.zones.items():
                turn_zones = self.pathfinder.zone_reservations.get(current_turn, {})
                print(f"[zone name: {zone_name} --> used_capacity: {turn_zones.get(zone_name, 0)} / max_capacity: {zone.max_drones}]")

            for drone_id, path in self.all_paths.items():
                prev_loc = self.get_location_at_time(path, current_turn - 1)
                curr_loc = self.get_location_at_time(path, current_turn)

                # Drones that waited, or already reached goal, are omitted
                if curr_loc and curr_loc != prev_loc:
                    moves_this_turn.append(f"D{drone_id}-{curr_loc}")
