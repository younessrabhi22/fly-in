...
class DroneVisualizer(arcade.Window):
    def __init__(self, engine):
        ...
        self.pixels_per_unit = int(
            min(120, x_pixels_per_unit, y_pixels_per_unit)
        )
        ...
        super().__init__(
            win_width, win_height, "Space-Time Dijkstra Drone Simulation"
        )

        self.turn = 0
        self.max_turn = max(
            (path[-1][0] for path in engine.all_paths.values() if path),
            default=0,
        )
        self.background_color = arcade.color.BLACK

    def get_zone_color(self, zone_name, zone):
        """Returns the color dynamically based on zone.color attribute."""
        color_map = {
    ...
    def get_pos(self, zone_name):
        ...
        return (
            100 + (z.x - self.min_x) * self.pixels_per_unit,
    ...
    def on_draw(self):
        ...
        for drone_id, path in self.engine.all_paths.items():
            ...
            if "-" in loc:
                parts = loc.split("-")
                p1, p2 = self.get_pos(parts[0]), self.get_pos(parts[1])
        ...
        # Draw turn counter
        arcade.draw_text(
            f"Turn: {self.turn} / {self.max_turn}",
            20,
            self.height - 30,
            arcade.color.WHITE,
            16,
